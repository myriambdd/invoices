...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get currency IDs
    const fromCurrencies = await query("SELECT id FROM currencies WHERE code = $1", [from_currency])
    const toCurrencies = await query("SELECT id FROM currencies WHERE code = $1", [to_currency])

    if (!fromCurrencies.length || !toCurrencies.length) {
    ...
    const exchangeRates = await query(`
      ...
      RETURNING *
    `, [fromCurrencies[0].id, toCurrencies[0].id, rate])

    return NextResponse.json(exchangeRates[0], { status: 201 })
  } catch (error) {
