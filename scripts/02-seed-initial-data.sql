...
-- Insert initial exchange rates (user will update these)
INSERT INTO exchange_rates (from_currency_id, to_currency_id, rate) 
SELECT 
    eur.id,
    tnd.id,
    3.25 -- Example rate, user will update
...
SELECT 
    usd.id,
    tnd.id,
    3.10 -- Example rate, user will update
...
SELECT 
    gbp.id,
    tnd.id,
    3.85 -- Example rate, user will update
